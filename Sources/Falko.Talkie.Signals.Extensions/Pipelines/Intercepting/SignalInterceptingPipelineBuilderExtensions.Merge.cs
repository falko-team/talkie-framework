...
{
    public static ISignalInterceptingPipelineBuilder Merge
    (
    ...
    )
    {
        return SignalInterceptingPipelineBuilder.Empty
            .InterceptSingleton(() => new MergeSignalInterceptor<Signal>(builder.Build(),
                mergeBuilder.Build()));
    }
    ...
    public static ISignalInterceptingPipelineBuilder<T> Merge<T>
    (
        this ISignalInterceptingPipelineBuilder builder,
        ISignalInterceptingPipelineBuilder<T> mergeSourceBuilder,
        ISignalInterceptingPipelineBuilder<T> mergeTargetBuilder
    ) where T : Signal
    {
        return builder.InterceptSingleton(() => new MergeSignalInterceptor<Signal>(mergeSourceBuilder.Build(),
            mergeTargetBuilder.Build()))
    ...
    ) where T : Signal
    {
        return SignalInterceptingPipelineBuilder<T>.Empty
            .InterceptSingleton(() => new MergeSignalInterceptor<T>(builder.Build(),
                mergeBuilder.Build()));
    }
    ...
    {
        return builder.InterceptSingleton(() => new MergeSignalInterceptor<T>(mergeSourceBuilder.Build(),
            mergeTargetBuilder.Build()));
    }
